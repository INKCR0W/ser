...
## 配置步骤：
## 准备工作--所有节点 
1、禁用交换分区，保证kubelet正常工作    vi /etc/fstab #修改fstab文件，注释掉交换分区 
...
## 第二步：调整内核参数--所有节点
```
写入内容1:    为Kubernetes提供所需的底层支持cat <<EOF | tee /etc/modules-load.d/k.conf
overlay
br_netfilter
EOF
...
注kubeadm config images list #查看所需镜像列表    
...
配置containerd.sock 作为容器 runtime-endpoint
...
## 第六步：加入集群--工作节点
1、主节点初始化完成后会生成类似下方的指令，把该内容复制到工作节点执行，即可加入集群
kubeadm join 10.4.220.105:6443 --token abcdef.0123456789abcdef \
> --discovery-token-ca-cert-hash sha256:8d987951412a23a94c7a131c99d39d0b0dd2596d7c80850033da30ad3874f367

kubectl get nodes #在主节点执行，可以查看加入情况
...
若calico.yaml文件中的版本与以上不同，需要自己修改为3.25.0，arm架构的修改为3.25.0-arm
kubectl apply -f calico.yaml 创建网络组件脚本
 
for i in *;do ctr -n k8s.io i import $i --platform=linux/arm64 ;done #导入网络组件所需的镜像文件 **#所有节点**
...
kubectl exec -it web-79f9c88bdc-djtfh bash #进入Linux6节点
...
kubectl exec -it web-79f9c88bdc-vhz4b bash #进入Linux7节点
echo "HelloKubernetes" > /usr/share/nginx/html/index.html
nginx -s reload #重启nginx
curl localhost #测试访问情况
