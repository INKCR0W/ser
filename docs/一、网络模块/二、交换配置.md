...
## 1小题
1.配置 SW1、SW2、SW3 的 Vlan，二层链路只允许下面 Vlan 通过，不限制 vlan1。（这个第二题有要求）<br />![image.png](https://cdn.nlark.com/yuque/0/2024/png/33622884/1708063099540-2623d3d5-127c-4647-9dff-43c5e5563cd1.png#averageHue=%23eeedec&clientId=u6a3e82b2-db38-4&from=paste&height=230&id=u821f7347&originHeight=287&originWidth=637&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=52053&status=done&style=none&taskId=ucc48fc8e-dd51-40cf-af7f-a08e35b582c&title=&width=509.6)<br />![image.png](https://cdn.nlark.com/yuque/0/2024/png/33622884/1708063117490-64e3db1c-3de2-44d2-85c2-b959778a4d94.png#averageHue=%23f0eeed&clientId=u6a3e82b2-db38-4&from=paste&height=291&id=u31ea61ac&originHeight=364&originWidth=636&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=68616&status=done&style=none&taskId=u474b2618-c942-478d-8933-8b851fd6d1b&title=&width=508.8)
```
全局配置模式下连续输入
VLAN 11  #创建VLAN 11
NAME CP1 #VLAN名称为CP1
...
```
## 2小题
2.SW1 和 SW2 之间利用三条裸光缆实现互通，其中一条裸光缆承载三层 IP 业务、一条裸光缆承载 VPN 业务、一条裸光缆承载二层业务。 用相关技术分别实现财务 1 段、财务 2 段业务路由表与其它业务路由表隔离，财务业务 VPN 实例名称为 Finance，RD 为 1:1。承载二层业务的只有一条裸光缆通道，配置相关技术，方便后续链路扩容与冗余备份，编号为 1，用 LACP 协议，SW1 为 active，SW2 为 passive；采 用目的、源 IP 进行实现流量负载分担。 
```
vrf配置
...
```
释义1、VRF（Virtual Routing and Rorwarding，虚拟路由转发）技术通过在一台三层转发设备上创建多张路由表实现数据或业务的隔离，常用于MPLS VPN、防火墙等一些需要实现隔离的应用场景。

2、LACP（链路汇聚控制协议）

3、loopback口作用：<br />3.1 建立路由邻居；<br />3.2 作为Router-ID；<br />3.3 虚拟隧道连接；<br />3.4 网络连通性测试；
## 3小题 
...
ip vrf forwarding Office
```
注：VRF概述VRF（Virtual Routing and Rorwarding，虚拟路由转发）技术通过在一台三层转发设备上创建多张路由表实现数据或业务的隔离，常用于MPLS VPN、防火墙等一些需要实现隔离的应用场景。
## 5小题
...
```
释义 SNMP（简单网络管理协议）简单来讲就是一个网络管理标准，方便不同品牌、不同类型的设备接入统一的网络管理系统。
## 6小题
...
```
释义镜像端口是用来监测端口流量的，由某个端口收集监测到的数据帧，方便用来分析流量数据、用来诊断网络故障等等

session 会话列表号/镜像事务号

source 镜像源端口就是被复制数据的端口<br />destiation 镜像目的端口就是接收数据的端口

tx #发送的流量<br />rx #接收的流量<br />both #发送和接收的流量
## 7小题
...
```
释义uldp #单向链路检测协议，单向链路就是物理链路正常的情况下，本端能和对端正常通信，而对端却不能跟本端正常通信
...
```
注：链路层发现协议（LLDP）链络设备可以通过在本地网络中发送LLDPDU（Link Layer Discovery Protocol Data Unit）来通告其他设备自身的状态。是一种能够使网络中的设备互相发现并通告状态、交互信息的协议。
